...
def get_base_dir():
    ...
    __file__ : 현재 실행 중인 파이썬 파일의 경로(파일 포함)
    다만, 주피터 노트북이나 일부 온라인 실행 환경에서는 __file__이 없을 수 있음.
    그래서 try-except를 사용해 __file__이 없으면 현재 작업 폴더 Path.cwd()를 사용.
    """
    try:
        base_dir = Path(__file__).resolve().parent  # Path(__file__) : 현재 파일 경로를 Path 객체로 변경
                                                    # Path(__file__)과 __file__ 의 값은 같음 형식이 다름
                                                    # .resoleve() : 절대 경로로 변경(최종 위치를 정확하게)
                                                    # .parent : 상위 경로
    except NameError:
        base_dir = Path.cwd()

    print('>' * 100)
    print(__file__,         type(__file__))         # 문자열 형식
    print(Path(__file__),   type(Path(__file__)))   # Path 객체
    print('>' * 100)

    data_dir = base_dir / "내장 모듈2_lesson_data"  # 새 폴더 생성
...
def lesson_random():
    # --------------------------------------------------------
    # random 모듈 : 예측하기 어려운 값 생성.
    # 기능 : 게임 아이템 뽑기, 발표자 선정, 팀 배정, 메뉴 추천
    # --------------------------------------------------------
    ...
    print(f"[응용] 아이템별 획득 개수(Counter 형식): {item_counter}")
    print(f"[응용] 아이템별 획득 개수(Dictionary 형식): {dict(item_counter)}")
...
def lesson_datetime():
    # --------------------------------------------------------
    # datetime 모듈 : 날짜와 시간을 계산하기 위한 표준 도구.
...
def lesson_time():
    # --------------------------------------------------------
    # time 모듈 : 프로그램이 실행되는 동안 흐르는 시간, 타이머
    # 기느 : 잠깐 멈추기, 코드 실행 시간 측정
    # --------------------------------------------------------
...
def lesson_os():
    # --------------------------------------------------------
    # os 모듈 : 파이썬 프로그램이 운영체제와의 소통
    # 기능 : 현재 작업 폴더 확인, 파일 목록 가져오기, 파일 정보 확인,환경 변수 읽기 등 컴퓨터의 실제 파일 시스템과 연결
    # 주의: os.system은 운영체제 명령을 직접 실행하므로, 주의
    # --------------------------------------------------------
    ...
    files = os.listdir('.')                         # 현재 폴더 내의 파일 목록 저장
...
def lesson_sys():
    # --------------------------------------------------------
    # sys 모듈 : 현재 실행 중인 파이썬 인터프리터 자체의 정보를 제공.
    # 기능 : 파이썬 버전, 실행 플랫폼, 명령줄 인수, import 경로 등
    # --------------------------------------------------------
    ...
    # [중급] 모듈 검색 경로 확인
    # import를 할 때 파이썬은 sys.path에 들어 있는 폴더들을 차례대로 찾아봄.
...
def lesson_json():
    ...
    # 딕셔너리와 리스트 구조를 저장하거나 다른 프로그램과 주고받기 쉬운 JSON 형식으로 변환
    # 기능 : 게임 랭킹, 사용자 설정, 설문 결과, 웹 API 데이터 처리 등
    ...
    with open(json_file, "w", encoding="utf-8") as file:
        json.dump(ranking_data, file, ensure_ascii=False, indent=4)     # dump는 파일에 직접 저장. dumps는 문자열로 생성.
    ...
    # [응용] 랭킹 데이터에서 1등 찾기
    # max 함수의 key 옵션을 사용하면 리스트 안의 딕셔너리 중 특정 값을 기준으로 최댓값을 찾을 수 있음.
    ...
    # [주의] JSON은 파이썬의 모든 자료형을 그대로 저장하지는 않음.
    # 예) 튜플을 JSON으로 저장하면 리스트로 변환해서 저장됨.
...
def lesson_collections():
    # --------------------------------------------------------
    # [함수 설명]
    # collections 모듈 : 리스트와 딕셔너리보다 편리한 자료구조를 제공
    # Counter : 개수 세기, defaultdict는 자동 그룹 만들기
    # deque : 빠른 대기열 처리
    # --------------------------------------------------------
    border("8. collections 모듈 - 채팅 로그와 대기열 분석")

    # [기초] Counter로 개수 세기
    # Counter : 리스트 안에 같은 값이 몇 번 나오는지 세어 주는 딕셔너리 비슷한 객체
    snack_votes = ["초코우유", "딸기우유", "초코우유", "바나나우유", "초코우유", "딸기우유"]
    vote_counter = Counter(snack_votes)
    print(f"[기초] 간식 투표 결과: {vote_counter}")
    print(f"[기초] 가장 인기 있는 간식 1개: {vote_counter.most_common(1)}")         # .most_common(1) :  가장 많이 등장하는 값 1개 찾기

    # [중급] 문자열에서도 Counter 사용 가능
    # 공백을 기준으로 단어를 나누면 단어 빈도 분석 가능
    ...
    # 일반 딕셔너리 : 존재하지 않는 key에 바로 append하면 오류 발생.
    # defaultdict(list) : 새 key 등장시 자동으로 빈 리스트 생성.
    # 동아리 이름별로 학생 이름 자동으로 모으기
    club_members = [
        ("밴드부", "민준"),
        ("코딩부", "서연"),
        ("밴드부", "도윤"),
    ...
    club_dict = defaultdict(list)       # defaultdict[] 생성, Key생성시 Value는 비어 있는 List 타입으로 자동 생성
    for club_name, student_name in club_members:
        club_dict[club_name].append(student_name)   # 같은 동아리 이름(Key)일 경우, 학생 이름을 리스트(Value)에 추가.
                                                    # {"밴드부":["민준", "도윤"]}
    ...
    # [중급] deque로 대기열 만들기
    # deque : 양쪽 끝에서 넣고 빼는 자료구조.(리스트은 오른쪽에서만 추가하고 꺼낼 수 있음)
    # 리스트의 pop(0)은 앞에서 꺼낼 때 느릴 수 있음.
    waiting_queue = deque(["1번 손님", "2번 손님", "3번 손님"])    # list => deque 자료구조로 변환
    waiting_queue.append("4번 손님")                                    # 오른쪽 끝에 추가
    first_customer = waiting_queue.popleft()                            # 왼쪽 끝에서 꺼내기
    print(f"[중급] 먼저 입장한 사람: {first_customer}")                 # 1번 손님
    print(f"[중급] 남은 대기열: {list(waiting_queue)}")            # ["2번 손님", "3번 손님","4번 손님"]
    ...
    monster_counter = Counter()                         # 몬스터를 만난 횟수
    result_by_monster = defaultdict(Counter)    # defaultdict[] 생성, Key생성시 Value는 비어 있는 Counter 타입으로 자동 생성

    for monster, result in battle_log:
        monster_counter[monster] += 1                           # 몬스터 만난 횟수 증가
        result_by_monster[monster][result] += 1                 # 해당 몬스터의 결과 횟수 증가

    print(f"[응용] 몬스터별 만난 횟수: {monster_counter}")      # Counter({'슬라임':3, '고블린':2, '드래곤':1})
    print("[응용] 몬스터별 승패:")
    for monster, result_counter in result_by_monster.items():
        print(f"  - {monster}: {dict(result_counter)}") # Counter를 Dictionary 로 변환해서 출력

    # [미니 과제]
    # 1. battle_log에 새로운 전투 기록 추가.
    # 2. 가장 많이 만난 몬스터를 most_common으로 찾기.

# ------------------------------------------------------------
# 9. pathlib: 파일 경로를 객체처럼 다루기
# ------------------------------------------------------------
def lesson_pathlib():
    # --------------------------------------------------------
    # [함수 설명]
    # pathlib 모듈 : 파일 경로를 (문자열이 아니라) Path 객체로 다룸
    # os.path 보다 사용하기 편함
    # 문자열을 / 연산자로 경로를 이어 붙임.
    # --------------------------------------------------------
    ...
    json_files = list(DATA_DIR.glob("*.json"))  # # glob("*.json") : 현재 폴더에서 .json으로 끝나는 파일을 찾음.
    ...
    # [미니 과제]
    # 1. DATA_DIR 안에 diary.txt를 만들고 오늘 배운 내용 저장.
    # 2. glob("*.txt")로 텍스트 파일 목록 출력.

# ------------------------------------------------------------
# 10. statistics: 평균, 중앙값 등 통계 계산
# ------------------------------------------------------------
def lesson_statistics():
    # --------------------------------------------------------
    # [함수 설명]
    # statistics 모듈 ： 기본 통계값
    # 기능 ： 평균, 중앙값, 최빈값, 표준편차 등
    # 활용 ： 수행평가 점수, 설문 결과, 게임 점수 분석 등
    # --------------------------------------------------------
    border("10. statistics 모듈 - 수행평가 점수 분석")

    scores = [88, 92, 75, 100, 67, 88, 91, 83, 95, 88]

    # [기초] 평균(mean)과 중앙값(median)
    mean_score = statistics.mean(scores)           # 평균값 
    median_score = statistics.median(scores)       # 중앙값 : 데이터를 크기순으로 정렬했을 때 가운데에 있는 값.
    print(f"[기초] 점수 목록: {scores}")
    print(f"[기초] 평균 점수: {mean_score:.2f}")
    print(f"[기초] 중앙값: {median_score}")

    # [중급] 최빈값(mode) : 가장 자주 등장한 값
    mode_score = statistics.mode(scores)
    print(f"[중급] 가장 많이 나온 점수: {mode_score}")

    # [중급] 표준편차(pstdev) : 데이터가 평균에서 얼마나 흩어져 있는지 나타내는 값
    # 값이 작을수록 평균 근처에 모여 있고, 클수록 차이가 큼.
    ...
    # [미니 과제]
    # 1. scores에 자신의 모둠 점수를 넣고 평균, 중앙값, 표준편차 구해기.
    # 2. 90점 이상 점수만 저장한 honor_scores 리스트 작성
